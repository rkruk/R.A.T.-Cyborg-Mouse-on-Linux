...
In the terminal as a root create separate file <span style="color: blue">`10-evdev.conf`</span> (check 'evdev location' example for specific file location and `10-evdev.conf` file for all necessary details). Why separate file and not make changes directly inside xorg.conf? Any potential update of X server will default xorg.conf file, and separate evdev.conf file will be untouched thus mouse will be operational.
